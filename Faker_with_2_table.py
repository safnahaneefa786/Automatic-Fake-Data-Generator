...
df = spark.read.option("delimiter", ",").option("header", True).csv("table.csv")
df.display()
df.createOrReplaceTempView('df_table')
df1 = spark.read.option("delimiter", ",").option("header", True).csv("attribute.csv")
df1.display()
...
count_d = spark.sql("select count(table_id) as total_table from df_table")
total_t = str(count_d.rdd.flatMap(lambda x: x).collect())
total_tables = re.sub(r"[\([{''})\]]", "", total_t)
#for loop for creating each table in the csv
for i in range (1,int(total_tables)+1):
    filter_value=i
    ...
    query4 = f'''SELECT count
                       FROM df_table
                       WHERE table_id = {filter_value}'''
    count = spark.sql(query4)
...
dbutils.fs.rm("{path which saved the cssv with }",recurse=True)

# COMMAND ----------

df = spark.read.option("delimiter", ",").option("header", True).csv("{}/shipper.csv")
df.display()

# COMMAND ----------

df = spark.read.option("delimiter", ",").option("header", True).csv("{}/order.csv")
df.display()
