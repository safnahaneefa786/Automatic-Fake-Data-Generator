...
dbutils.fs.rm("/FileStore/shared_uploads/safna.h@reflectionsinfos.com/faker_utility/payment.csv",recurse=True)
...
# %sql
df = spark.read.option("delimiter", ",").option("header", True).csv("{path}+order.csv")
df.display()

# COMMAND ----------

# %sql
df = spark.read.option("delimiter", ",").option("header", True).csv("{path}./payment.csv")
df.display()
